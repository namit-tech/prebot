(() => {
  ...
  // mobile-bridge.js
  console.log("\u{1F50C} Mobile Bridge: Loading Capacitor Speech Plugin...");
  window.CapacitorSpeech = SpeechRecognition2;
  window.isSpeechPluginAvailable = async () => {
    try {
      const available = await SpeechRecognition2.available();
      console.log("\u{1F3A4} Speech Plugin Available:", available);
      return available;
    } catch (e) {
      console.error("\u274C Speech Plugin Check Failed:", e);
      return false;
    }
  };
  console.log("\u2705 Mobile Bridge: Plugin exposed as window.CapacitorSpeech");
})();
